...
class CreateClass(LoginRequiredMixin,CreateView):
	...
	# Transaccon a la base de datos, si alguno no se puede guardar, hace un rollback a las demas tablas
	@transaction.atomic
	...
	def form_valid(self, form):
		...
		# Indicar que le pertenece al usuario logueado
...
def admin_only(function):  
	def wrap(request, *args, **kwargs):
	  project = get_object_or_404(Project,slug=kwargs['slug']) #Encontrar el usuario
	  if not project.user_has_permission(request.user): #Verificar los permisos
...
def edit(request, slug=''):
	...
	forms_status = StatusChoiceForm(request.POST or None, initial = {'status': project.get_id_status()})
...
def add_contributor(request, slug, username):
	...
	if not project.projectuser_set.filter(user=user).exists():
		project.projectuser_set.create(user=user,permission=ProjectPermission.contributor_permission())
...
def user_contributor(request, slug, username):
	project = get_object_or_404(Project, slug=slug)
	user = get_object_or_404(User, username=username)
	permission = get_object_or_404(ProjectUser, user=user, project=project) # Permiso que tiene actualmente el usuario

	form = PermissionProject(request.POST or None,
													initial={'permission' : permission.permission_id })

	if request.method == 'POST' and form.is_valid():
		selection_id = form.cleaned_data['permission'].id #Obtener el id que selecciono el usuario
		if selection_id != permission.id and permission.valida_change_permission(): # Si el id es diferente al que ya tenia
